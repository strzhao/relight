...
describe("PersonStrip — 点击交互（SSR 静态校验）", () => {
  it("每个头像应渲染为可点击元素（button 或 a 或带 role='button' 的 div）", async () => {
    ...
    const isClickable =
      /<button[^>]*>[^]*?\/api\/persons\/p-clickable\/avatar\.jpg/.test(html) ||
      /<a[^>]*>[^]*?\/api\/persons\/p-clickable\/avatar\.jpg/.test(html) ||
      /role="button"[^>]*>[^]*?\/api\/persons\/p-clickable\/avatar\.jpg/.test(html) ||
