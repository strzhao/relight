...
const faces = db
  .prepare(
    "SELECT id, photo_id, embedding, detection_score, bbox_w, bbox_h FROM faces WHERE person_id = ? ORDER BY detection_score DESC",
  )
  .all(personId);
...
const rows = faces.map((f) => {
  ...
  return {
    ...
    sim,
    size: Math.round((f.bbox_w + f.bbox_h) / 2),
  };
});
...
function stat(name, arr) {
  ...
  console.log(`\n=== ${name} (n=${arr.length}) ===`);
  console.log(
    `cosine sim: min=${sims[0].toFixed(3)} p25=${q(sims, 0.25).toFixed(3)} median=${q(sims, 0.5).toFixed(3)} p75=${q(sims, 0.75).toFixed(3)} max=${sims[sims.length - 1].toFixed(3)}`,
  );
  console.log(
    `det score:  min=${scores[0].toFixed(3)} median=${q(scores, 0.5).toFixed(3)} max=${scores[scores.length - 1].toFixed(3)}`,
  );
...
console.log(
  `person ${personId.slice(0, 8)} (${person.name}/${person.nickname}) — 总 face ${total}, member_count=${person.member_count}`,
);
...
console.log("\n=== 不准的逐条 (按 sim 降序) ===");
for (const r of [...wrong, ...unmarked].sort((a, b) => b.sim - a.sim)) {
  console.log(
    `  ${r.id} sim=${r.sim.toFixed(3)} score=${r.score.toFixed(2)} size=${r.size}px verdict=${r.v}`,
  );
}

console.log("\n=== 准的低 sim 边缘 (前 5 最低) ===");
for (const r of [...right].sort((a, b) => a.sim - b.sim).slice(0, 5)) {
...
for (const t of [0.55, 0.6, 0.65, 0.7]) {
  ...
  console.log(
    `  threshold=${t}: 过滤 wrong ${filteredWrong}/${wrong.length}, 误伤 right ${lostRight}/${right.length}`,
  );
}
