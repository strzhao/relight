...
for (const p of persons) {
  ...
  try {
    ...
    if (isHeic) {
      ...
      buf = await sharp(Buffer.from(d.data), {
    ...
    const avatarAbs = await generateAutoAvatar(
      buf,
      { x: best.bbox_x, y: best.bbox_y, w: best.bbox_w, h: best.bbox_h },
      meta.width,
      meta.height,
      p.id,
    );
    ...
    db.prepare(
      "UPDATE persons SET representative_face_id=?, avatar_path=?, updated_at=? WHERE id=?",
