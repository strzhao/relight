...
function hasUniqueConstraint(
  ...
  for (const [key, value] of Object.entries(tableAny)) {
    if (typeof value === "object" && value !== null) {
      ...
      if (
        typeof valAny.getName === "function" &&
        Array.isArray(valAny.columns)
      ) {
...
describe("photos 表复合 UNIQUE 约束 — 验收测试（设计文档 §1）", () => {
  ...
  describe("复合 UNIQUE 约束 (storage_source_id, file_path)", () => {
    it("应定义复合唯一约束覆盖 storage_source_id 和 file_path 两列", () => {
      ...
      // 检查 Symbol 配置
      const symbolConfig = tableAny[
        Symbol.for("drizzle:SQLiteTable")
      ...
      const hasTargetConstraint = foundConstraints.some((c) => {
        ...
        return (
          cols.includes("storage_source_id") && cols.includes("file_path")
        );
      });
      ...
      if (symbolConfig) {
        ...
        if (uq) {
          for (const c of uq) {
            ...
            if (
              cols.includes("storage_source_id") &&
              cols.includes("file_path")
            ) {
