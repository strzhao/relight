...
// 后端包根目录（从源码目录上溯一级）。子进程测试用它作为 cwd，避免硬编码绝对仓库路径
// （此前写死 /Users/stringzhao/... 在 CI runner 上不存在，导致子进程跑不起来 → 测试失败）。
const BACKEND_DIR = path.resolve(RELIGHT_BACKEND_SRC, "..");
...
describe("契约 3: WHISPER_INITIAL_PROMPT 合成 — 日志格式契约（子进程路径）", () => {
  it("batch-index 应输出 [batch-index] initialPrompt auto-merged: 日志行", async () => {
    ...
    try {
      ...
      const out = execSync(
        `WHISPER_INITIAL_PROMPT='测试环球影城' pnpm exec tsx ${BATCH_INDEX_PATH} ${testDir} --out /tmp/vlog-test-manifest-accept.json --skip-transcribe --no-ai 2>&1 || true`,
        { encoding: "utf-8", timeout: 30_000, cwd: BACKEND_DIR },
      );
      // 如果实现了合并逻辑，应输出该日志
  ...
  it("不传 WHISPER_INITIAL_PROMPT 时，自动构造的 prompt 不含 undefined 字样", async () => {
    ...
    if (fn) {
      ...
      try {
        ...
        execSync(`mkdir -p ${testDir}`, { stdio: "ignore" });
        const out = execSync(
          `pnpm exec tsx ${BATCH_INDEX_PATH} ${testDir} --out /tmp/vlog-test-manifest-accept2.json --skip-transcribe --no-ai 2>&1 || true`,
          { encoding: "utf-8", timeout: 30_000, cwd: BACKEND_DIR },
        );
        if (out.includes("[batch-index] initialPrompt auto-merged:")) {
