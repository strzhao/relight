...
function createPhotosTable(db: Database.Database): void {
  ...
  db.prepare("INSERT INTO storage_sources (id, name, type, root_path) VALUES (?, 'test', 'local', '/t')").run(
    "source-001",
  );
}
...
describe("CC-2：photos 表 schema 包含 14+1 列（红队验收）", () => {
  ...
  describe("14+1 列全部 nullable", () => {
    it("latitude 允许 NULL 值写入", () => {
      ...
      const row = sqlite
        .prepare("SELECT latitude FROM photos WHERE id = ?")
        .get("p-null-gps") as { latitude: number | null };
  ...
  describe("写入后幂等", () => {
    ...
    it("回填后数据正确写入 14 列", () => {
      ...
      sqlite
        ...
        .run(35.0, 139.0, 50.0, 90.0, "+08:00", "Sony", "ILCE-7M4", "85mm", 85.0, 85, 800, 0.001, 2.8, "Lightroom", now);

      const row = sqlite
        .prepare("SELECT * FROM photos WHERE id='img-verify'")
