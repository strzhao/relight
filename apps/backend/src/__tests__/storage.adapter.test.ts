  ...
  describe("getMetadata — sharp 元数据提取（修复 2）", () => {
    ...
    it("getMetadata 返回的对象应包含 width/height 及 EXIF 元数据字段", async () => {
      ...
      const allowedKeys = new Set([
        ...
        "latitude", "longitude", "altitude", "gpsImgDirection",
        "offsetTime", "cameraMake", "cameraModel", "lensModel",
        "focalLength", "focalLength35mm", "iso", "exposureTime", "fNumber", "software",
        // 视频字段（图片路径不设置，但 interface 兼容）
        "mediaType", "durationSec", "videoCodec", "videoFps",
      ]);
