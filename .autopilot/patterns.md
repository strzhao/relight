...
### [2026-05-05] 图片端点错误响应必须返回图片格式（SVG 占位图），不能用 JSON

<!-- tags: backend, image, error-handling, api-design, img-tag -->

**Scenario**: lightbox 中所有原图显示"加载失败"，但缩略图正常。根因是两个图片端点对错误处理不一致：`/thumbnail` 在文件缺失时返回 SVG 占位图（`image/svg+xml`），`<img>` 正常渲染；`/original` 返回 `c.json({error}, 404/500)`，JSON 被 `<img>` 标签视为无效图片数据，触发 `onError`。

**Lesson**: 所有供 `<img>` 标签直接使用的端点，错误响应必须返回有效图片（SVG 占位图 + `Content-Type: image/svg+xml`），不能返回 JSON。`<img>` 标签只认图片 MIME 类型，收到 JSON 即视为加载失败，即使 HTTP 状态码正确。项目中多个图片端点应保持此行为一致。

**Evidence**: 存储源不可达时 `/original` 返回 404 JSON → lightbox 全部"加载失败"；`/thumbnail` 返回 SVG 占位图 → 缩略图网格正常。修复：`/original` 三处错误路径（照片不存在、文件不存在、读取失败）均改为 `c.body(svg, 200, {"Content-Type": "image/svg+xml"})`。
